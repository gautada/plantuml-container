...
WORKDIR /opt
RUN wget https://dlcdn.apache.org/tomcat/tomcat-10/v10.0.22/bin/apache-tomcat-10.0.22.tar.gz \
 && tar -zxf apache-tomcat-10.0.22.tar.gz \
 && mv /opt/apache-tomcat-10.0.22 /opt/tomcat10 \
...
RUN /bin/mkdir -p /opt/$USER /var/backup /tmp/backup /opt/backup \
 ...
 && /bin/chown $USER:$USER -R /opt/tomcat10 /opt/$USER /etc/backup /var/backup /tmp/backup /opt/backup
